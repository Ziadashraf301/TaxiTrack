...
class MLTrainingPipeline:
    ...
    def run(
        ...
        """
        Execute full training workflow using parameters from configs/ml_config.yaml.
        """
        ...
        s_date = start_date or cfg_data.start_date
        e_date = end_date or cfg_data.end_date
        ...
        logger.info(
            ...
            f"  TRAINING WINDOW:    {s_date} ➔ {e_date}\n"
            f"  VALIDATION MONTHS:  {t_months}\n"
        ...
        df_raw = self.repo.get_demand_data(start_date=s_date, end_date=e_date)
        if df_raw.empty:
            raise ValueError(f"No training data found in ClickHouse for window [{s_date}, {e_date}].")
        ...
        logger.info(f"Executing Lookback Buffer split & feature engineering (validation holdout: {t_months} months)...")
        X_train, X_val, y_train, y_val = feature_engineer.prepare_train_val_split(
            df_raw, test_months=t_months
        )
        ...
        # Step 7: ONNX Export & Parity Validation
        onnx_path = None
        onnx_benchmarks = {}
        if export_onnx and hasattr(forecaster, "model"):
            try:
                ...
                ONNXModelExporter.validate_numerical_parity(
        ...
        run_name = f"{m_type.lower()}_train_{e_date}"
